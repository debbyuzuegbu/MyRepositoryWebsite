...
function SingleRepo() {
  ...
  return <div style={{display:"flex", padding: "4rem", height: "100vh", marginLeft: "30%"}}>{
    repo && repo.map(
        (per) => <div key={per.id} style={{backgroundColor: "#0b0e14", color:"#fff", padding: "5rem"}}>
            {per.name}
            <p><img style={{height: "10rem"}} alt="Debby's face" src={per.owner.avatar_url}/></p>
            <p>{per.owner.login}</p>
            ...
            <a href={per.svn_url} style={{color: "#fff"}}>view repo</a>
